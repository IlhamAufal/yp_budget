<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200/80 dark:border-gray-800 shadow-xs space-y-5">
        <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Unggah file Excel hasil pengisian template Sales Domestic.</p>
        </div>
        <div class="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-8 text-center hover:border-emerald-500 transition-colors bg-gray-50/50 dark:bg-gray-800/40 space-y-4">
            ...
            <div class="flex flex-wrap items-center justify-center gap-3 pt-2">
                <button type="button" @click="$refs.domesticUploadFile.click()" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl shadow-xs transition-all flex items-center gap-2 active:scale-[0.98]">
                    <i class="fa-solid fa-folder-open"></i>
                    <span>Pilih Berkas Excel</span>
                </button>
                <button type="button" @click="uploadFile($refs.domesticUploadFile)" class="px-5 py-2.5 bg-[#2F3185] hover:bg-[#25276d] text-white text-xs font-semibold rounded-xl shadow-xs transition-all flex items-center gap-2 active:scale-[0.98]">
    ...
    <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200/80 dark:border-gray-800 shadow-xs space-y-5 flex flex-col justify-between">
        <div class="space-y-5">
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Agregasi ulang data dari Regional ke tabel Summary Domestic SKU.</p>
            </div>
            <div class="p-4 rounded-xl bg-amber-50/80 dark:bg-amber-950/30 border border-amber-200/80 dark:border-amber-900/50 text-xs text-amber-900 dark:text-amber-200 space-y-2">
                ...
                <p class="text-gray-600 dark:text-gray-300">
                    Proses ini akan mengonsolidasikan seluruh transaksi detail regional ke dalam ringkasan channel domestic untuk tahun aktif <strong><?= esc($workingYear) ?></strong>.
                </p>
            </div>
        </div>
        <div>
