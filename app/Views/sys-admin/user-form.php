...
<div id="userFormContainer">
  <form id="userFormElement" class="p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        <input type="text" id="uf_user_username" required maxlength="10" value="<?= esc($isEdit ? ($user['user_username'] ?? '') : '') ?>" placeholder="Contoh: budi01" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm font-mono font-bold text-gray-900 focus:border-brand-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-colors" />
      </div>
      <div>
        ...
        <input type="text" id="uf_user_name" required value="<?= esc($isEdit ? ($user['user_name'] ?? '') : '') ?>" placeholder="Contoh: Budi Santoso" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm font-semibold text-gray-900 focus:border-brand-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-colors" />
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        <input type="email" id="uf_user_email" value="<?= esc($isEdit ? ($user['user_email'] ?? '') : '') ?>" placeholder="Contoh: budi@company.com" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm text-gray-900 focus:border-brand-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-colors" />
      </div>
      <div>
        <label class="block text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Password <span class="text-red-500" id="uf_password_required_star"><?= $isEdit ? '' : '*' ?></span><span class="normal-case font-medium text-gray-400" id="uf_password_hint"><?= $isEdit ? ' (kosongkan jika tidak diubah)' : '' ?></span></label>
        <input type="password" id="uf_user_password" <?= $isEdit ? '' : 'required' ?> minlength="6" placeholder="Minimal 6 karakter" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm font-mono text-gray-900 focus:border-brand-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-colors" />
      </div>
    </div>
      ...
      <div class="relative" id="uf_role_dropdown_container">
        ...
        <div id="uf_role_menu" class="absolute z-20 mt-2 w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-xl overflow-hidden hidden">
          <div class="relative border-b border-gray-100 dark:border-gray-800">
            ...
            <input type="text" id="uf_role_search" placeholder="Cari role..." class="w-full bg-transparent py-3 pl-11 pr-4 text-sm text-gray-800 dark:text-white placeholder:text-gray-400 focus:outline-none" />
          </div>
          <div id="uf_role_list" class="max-h-56 overflow-y-auto py-1.5">
            <?php foreach ($roleOptions as $role): ?>
              <button type="button" data-role-id="<?= (int) $role['role_id'] ?>" data-role-name="<?= esc($role['role_name_idn']) ?>" class="uf-role-option w-full flex items-center justify-between gap-3 px-4 py-2.5 text-sm text-left transition-colors text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800">
    ...
    <div>
      ...
      <div class="relative" id="uf_obj_role_dropdown_container">
        ...
        <button type="button" id="uf_obj_role_toggle_btn" class="w-full flex items-center justify-between gap-3 rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm text-left font-semibold text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-colors">
          <span class="flex items-center gap-2.5">
        ...
        <div id="uf_obj_role_menu" class="absolute z-20 mt-2 w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-xl overflow-hidden hidden">
          <div class="relative border-b border-gray-100 dark:border-gray-800">
            <i class="fa-solid fa-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none"></i>
            <input type="text" id="uf_obj_role_search" placeholder="Cari object role..." class="w-full bg-transparent py-3 pl-11 pr-4 text-sm text-gray-800 dark:text-white placeholder:text-gray-400 focus:outline-none" />
          </div>
          <div id="uf_obj_role_list" class="max-h-56 overflow-y-auto py-1.5">
            <?php foreach ($roleOptionsObj as $role): ?>
              <button type="button" data-obj-role-id="<?= (int) $role['role_id'] ?>" data-obj-role-name="<?= esc($role['role_name_idn']) ?>" class="uf-obj-role-option w-full flex items-center justify-between gap-3 px-4 py-2.5 text-sm text-left transition-colors text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800">
                <span><?= esc($role['role_name_idn']) ?></span>
    ...
    <section class="rounded-2xl border border-indigo-200 bg-indigo-50/40 dark:border-indigo-900/70 dark:bg-indigo-500/5 overflow-hidden">
      ...
      <div class="p-5 space-y-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p class="text-sm text-gray-600 dark:text-gray-300"><span id="uf_custom_menu_count" class="font-bold text-gray-900 dark:text-white"><?= count($initialMenuIds) ?></span> menu terpilih dari <?= count($menuList) ?> menu</p>
          <div class="flex items-center gap-2">
            <button type="button" id="uf_select_all_menus_btn" class="rounded-lg border border-indigo-200 bg-white px-3 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-50 dark:border-indigo-900 dark:bg-gray-900 dark:text-indigo-300 dark:hover:bg-indigo-500/10 transition-colors"><i class="fa-solid fa-check-double text-[10px]"></i> Pilih Semua</button>
            <button type="button" id="uf_clear_all_menus_btn" class="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-bold text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 transition-colors"><i class="fa-solid fa-eraser text-[10px]"></i> Kosongkan</button>
          </div>
        </div>
          ...
          <?php foreach ($groupedMenus as $group => $groupMenus): ?>
            <div>
              <div class="flex items-center justify-between mb-3">
                <h4 class="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 flex items-center gap-2"><i class="fa-solid fa-layer-group"></i> <?= esc($group) ?></h4>
              ...
              <div class="space-y-1.5">
                <?php foreach ($groupMenus as $menu): ?>
                  ...
                  <label class="flex items-start gap-3 rounded-xl border border-indigo-100 bg-white/70 p-3.5 cursor-pointer hover:border-indigo-300 dark:border-gray-700 dark:bg-gray-900/70 dark:hover:border-indigo-700 transition-colors" style="margin-left: <?= min(4, $level - 1) * 1.25 ?>rem">
                    <input type="checkbox" name="menu_ids[]" value="<?= (int) $menu['menu_id'] ?>" class="uf-menu-checkbox mt-0.5 h-4 w-4 rounded border-gray-300 text-indigo-500 focus:ring-indigo-500/30" <?= in_array((int) $menu['menu_id'], $initialMenuIds, true) ? 'checked' : '' ?> />
                    <span class="flex items-center gap-2.5">
                      <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-50 border border-indigo-100 text-indigo-500 dark:bg-indigo-500/10 dark:border-indigo-900">
                        <i class="<?= esc($menu['menu_icon'] ?: 'fa-regular fa-circle') ?> text-xs"></i>
                      </span>
                      <span>
                        <span class="block text-sm font-bold text-gray-800 dark:text-gray-100"><?= esc($menu['menu_name_idn']) ?></span>
                        <span class="block text-xs text-gray-400 dark:text-gray-500"><?= esc($menu['menu_link'] ?: '-') ?></span>
                      </span>
                    </span>
                  </label>
                <?php endforeach; ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div>
        ...
        <select id="uf_user_active" required class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm font-bold text-gray-900 focus:border-brand-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-colors">
      ...
      <div>
        ...
        <select id="uf_user_block" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm font-bold text-gray-900 focus:border-brand-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-colors">
    ...
    <div class="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 dark:border-gray-800">
      <button type="button" id="uf_cancel_btn" class="rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-5 py-3 text-sm font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Batal</button>
      <button type="submit" id="uf_submit_btn" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-500 px-6 py-3 text-sm font-bold text-white shadow-md hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-500/40 disabled:opacity-50 transition-colors">
