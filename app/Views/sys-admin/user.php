...
<div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6 space-y-6" x-data="userPage()" @modal-saved.window="if($event.detail.entity === 'user') setTimeout(() => location.reload(), 600)">
  ...
  <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200/80 dark:border-gray-800 shadow-xs">
    <div>
      <div class="flex items-center gap-2 text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">
        ...
        <i class="fa-solid fa-chevron-right text-[9px] text-gray-400"></i>
        <span class="text-[#2F3185] font-bold">User Management</span>
      </div>
  ...
  <div class="rounded-2xl border border-gray-200/80 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 shadow-xs">
    <form method="GET" action="<?= base_url('sys-admin/user') ?>" class="flex flex-wrap items-end gap-3">
      <div class="flex-1 min-w-[240px]">
        <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Pencarian User</label>
        <input
          type="text"
          name="search"
      ...
      <div class="min-w-[160px]">
        <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Role</label>
        <select
          name="role_id"
  ...
  <div class="space-y-4">
    <div>
      <h3 class="text-base font-bold text-gray-900 dark:text-white tracking-tight">Daftar Pengguna Sistem (User List)</h3>
      <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Kelola data akun, otorisasi menu, dan pembagian hak akses departemen/object.</p>
    </div>
        ...
        <table class="w-full text-left border-collapse text-xs min-w-[900px]">
          <thead class="bg-[#2F3185] text-white font-semibold text-xs border-b border-white/20">
            <tr class="bg-[#2F3185] text-white font-semibold">
              ...
              <th class="py-3 px-5 text-white font-semibold">User</th>
              <th class="py-3 px-5 text-white font-semibold">Role</th>
              <th class="py-3 px-5 text-white font-semibold">Object</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800 text-xs">
            <?php if (empty($rows)): ?>
              <tr>
                <td colspan="6" class="py-20 text-center text-gray-400 dark:text-gray-500 font-sans">
                  <div class="flex flex-col items-center justify-center gap-3">
                    ...
                    <p class="font-semibold text-gray-800 dark:text-gray-200 text-sm">Tidak ada user ditemukan</p>
                ...
                <tr x-show="isRowVisible(<?= $index ?>)" class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors">
                  ...
                  <td class="py-3.5 px-5">
                    <div class="flex items-center gap-3">
                      <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-[#2F3185]/10 text-[#2F3185] dark:bg-[#2F3185]/20 dark:text-indigo-400 font-bold">
                      ...
                      <div>
                        <div class="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                          ...
                          <?php if ((int) session()->get('user_id') === (int) $r['user_id']): ?>
                            <span class="rounded-md bg-[#2F3185]/10 px-1.5 py-0.5 text-[10px] font-bold text-[#2F3185] dark:bg-[#2F3185]/20 dark:text-indigo-400">Anda</span>
                          <?php endif; ?>
                        </div>
                  ...
                  <td class="py-3.5 px-5">
                    <?php if (empty($menuRoleIds)): ?>
                      <?php if ($isAdminRole): ?>
                        <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400">
                          <i class="fa-solid fa-crown text-[10px]"></i> Admin
                        </span>
                      <?php else: ?>
                        <span class="inline-flex items-center gap-1.5 rounded-lg bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                    ...
                    <?php else: ?>
                      <div class="flex flex-wrap items-center gap-1.5">
                        <?php foreach ($menuRoleIds as $rid): ?>
                          ...
                          ?>
                          <span class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-0.5 text-xs font-semibold <?= $isMenuAdmin ? 'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400' : 'bg-[#2F3185]/10 text-[#2F3185] dark:bg-[#2F3185]/20 dark:text-indigo-400' ?>">
                    ...
                    <?php else: ?>
                      <div class="flex flex-wrap items-center gap-1.5">
                        <?php foreach ($objRoleIds as $i => $oid): ?>
                          <span class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-0.5 text-xs font-semibold bg-teal-50 text-teal-600 dark:bg-teal-500/10 dark:text-teal-400" title="Object role (scope data)">
  ...
  >
    ...
    >
      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
        <div>
          <h3 class="text-base font-bold text-gray-900 dark:text-white">Reset Password</h3>
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Atur ulang kata sandi untuk <span x-text="resetForm.userName" class="font-semibold text-gray-700 dark:text-gray-200"></span></p>
        </div>
        <button
          type="button"
      ...
      <form @submit.prevent="doResetPassword()" class="p-6 space-y-4">
        ...
        <div class="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 dark:border-gray-800">
          ...
          <button
            type="submit"
            :disabled="resetting"
            class="bg-amber-600 hover:bg-amber-700 text-white font-semibold rounded-xl px-5 py-2.5 shadow-xs transition-all inline-flex items-center justify-center gap-2 active:scale-[0.98] text-xs disabled:opacity-50"
          >
