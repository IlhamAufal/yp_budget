<div x-data="opexSellingEntryTab()" x-init="init()" class="space-y-6">
    ...
    <div class="rounded-2xl border border-amber-200/80 bg-amber-50/70 p-4 dark:border-amber-900/50 dark:bg-amber-950/30 shadow-xs">
        <div class="flex items-start gap-3">
            ...
            <div>
                <h4 class="font-bold text-amber-900 dark:text-amber-200 text-xs mb-1">Pemberitahuan penting:</h4>
    ...
    <div class="rounded-2xl border border-gray-200/80 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 shadow-xs">
        ...
        <div class="relative w-full max-w-2xl">
            <div
                ...
                class="flex w-full items-center justify-between rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3.5 py-2.5 text-xs text-gray-800 cursor-pointer dark:text-white transition-all focus:border-[#2F3185] focus:ring-2 focus:ring-[#2F3185]/20 focus:bg-white"
            >
            ...
            <div
                ...
                x-cloak
                class="absolute left-0 top-full z-40 mt-1 w-full rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-800 overflow-hidden"
            >
                <div class="p-3 border-b border-gray-100 dark:border-gray-700">
                    <input
                        ...
                        class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 px-3.5 py-2 text-xs focus:border-[#2F3185] focus:ring-2 focus:ring-[#2F3185]/20 focus:outline-none dark:text-white"
                    >
                </div>
                <ul class="max-h-48 overflow-y-auto text-xs p-1.5">
                    <template x-for="item in costCenters.filter(c => c.name.toLowerCase().includes(searchCostCenter.toLowerCase()))" :key="item.id">
                        <li
                            ...
                            class="cursor-pointer px-3.5 py-2 rounded-xl hover:bg-[#2F3185]/10 hover:text-[#2F3185] dark:hover:bg-[#2F3185]/20 dark:hover:text-indigo-400 transition-colors"
                            :class="selectedCostCenter === item.id ? 'bg-[#2F3185] text-white font-bold' : 'text-gray-700 dark:text-gray-300'"
                            x-text="item.name"
                        ></li>
                    </template>
                </ul>
    ...
    <div class="space-y-4">
        ...
        <div>
            <h3 class="text-base font-bold text-gray-900 dark:text-white tracking-tight">Daftar Akun Header OPEX Selling (Actual)</h3>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Ringkasan actual biaya penjualan sebelum pengisian detail budget.</p>
        </div>
                ...
                <table class="w-full text-left text-xs text-gray-600 dark:text-gray-300 border-collapse min-w-[1000px] whitespace-nowrap">
                    <thead class="bg-[#2F3185] text-white text-xs font-semibold border-b border-white/20">
                        <tr class="border-b border-white/20 bg-[#2F3185] text-white font-semibold">
                            ...
                            <th rowspan="2" class="border-r border-white/20 py-3 px-4 min-w-[260px] text-white font-semibold">Selling Expense</th>
                            <th colspan="8" class="border-r border-white/20 py-2 px-3 text-center bg-[#25276d] text-white font-semibold">Actual</th>
                            <th rowspan="2" class="py-3 px-4 text-right min-w-[110px] text-white bg-[#25276d] font-bold">Total</th>
                        </tr>
                        <tr class="border-b border-white/20 bg-[#25276d] text-white text-xs font-semibold">
                            <template x-for="m in actualMonths" :key="m">
                                <th class="border-r border-white/20 py-2 px-2.5 text-right text-white font-semibold" x-text="m"></th>
                            </template>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors">
                                <td class="border-r border-gray-200 dark:border-gray-800 py-2.5 px-4 text-center font-sans">
                                    <a
                                        ...
                                        title="Entry"
                                        :class="row.indicator === 'sudah' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-[#2F3185] hover:bg-[#25276d]'"
                                ...
                                <td class="border-r border-gray-200 dark:border-gray-800 py-2.5 px-4 font-sans font-medium text-gray-900 dark:text-white" x-text="row.cost_center_header"></td>
                                <template x-for="m in actualMonthKeys" :key="m">
                                    <td class="border-r border-gray-200 dark:border-gray-800 py-2.5 px-2.5 text-right" x-text="fmtBudget(row[m])"></td>
                                </template>
                                <td class="py-2.5 px-4 text-right font-bold text-gray-900 dark:text-white" x-text="fmtBudget(row.total)"></td>
                            </tr>
                        </template>
                    </tbody>
                    <tfoot>
                        <template x-if="entries.length > 0">
                            <tr class="bg-[#2F3185]/10 dark:bg-[#2F3185]/20 font-bold text-gray-900 dark:text-white border-t-2 border-gray-300 dark:border-gray-700 text-xs font-mono">
                                <td colspan="2" class="border-r border-gray-300 dark:border-gray-700 py-3 px-4 font-sans font-bold">Grand Total</td>
                                <template x-for="m in actualMonthKeys" :key="'ft_' + m">
                                    <td class="border-r border-gray-300 dark:border-gray-700 py-3 px-2.5 text-right" x-text="fmtBudget(columnTotal(m))"></td>
                                </template>
                                <td class="py-3 px-4 text-right text-[#2F3185] dark:text-indigo-400 font-bold" x-text="fmtBudget(grandTotal())"></td>
                            </tr>
                        </template>
                    </tfoot>
                </table>
