...
<div x-data="opexGaDetailApp()" x-init="init()" class="space-y-6">
    ...
    <?= $this->include('opex_ga/partials/modal_item_detail') ?>
...
function opexGaDetailApp() {
    ...
    return {
        ...
        loadMatrix() {
            ...
            fetch(`<?= base_url('opex-ga/getDetailMatrix') ?>?dept=${dept}&header=${header}`)
                .then(r => r.json())
                .then(res => {
        ...
        openAddItemModal() {
            this.editIndex = -1;
            this.formItem = { nama_barang: '', jan: 0, feb: 0, mar: 0, apr: 0, may: 0, jun: 0, jul: 0, aug: 0, sep: 0, oct: 0, nov: 0, dec: 0 };
        ...
        saveItemModal() {
            if (this.formItem.nama_barang.trim() === '') {
