    ...
    <div class="rounded-2xl border border-amber-200/80 bg-amber-50/70 p-4 dark:border-amber-900/50 dark:bg-amber-950/30">
        <div class="flex items-start gap-3">
            ...
            <div class="text-xs text-amber-800 dark:text-amber-300">
                <h4 class="text-xs font-bold uppercase tracking-wider text-amber-900 dark:text-amber-200">Peringatan / Warning Information:</h4>
    ...
    <div class="rounded-2xl border border-gray-200/80 bg-white shadow-xs overflow-hidden dark:border-gray-800 dark:bg-gray-900">
            ...
            <table class="w-full text-left text-xs text-gray-600 dark:text-gray-300 border-collapse">
                <thead>
                    <tr class="border-b border-gray-200/80 dark:border-gray-800 bg-gray-50/75 dark:bg-gray-800/50 text-[11px] font-bold capitalize tracking-normal text-gray-500 dark:text-gray-400">
                        <th rowspan="2" class="border-r border-gray-200/80 dark:border-gray-800 px-4 py-3 text-center w-20">Entry</th>
                        <th rowspan="2" class="border-r border-gray-200/80 dark:border-gray-800 px-4 py-3 min-w-[240px]">General Administrative Expense</th>
                        <th colspan="8" class="border-r border-gray-200/80 dark:border-gray-800 px-2 py-2 text-center">Actual</th>
                        <th rowspan="2" class="px-4 py-3 text-right">Total</th>
                    </tr>
                    <tr class="border-b border-gray-200/80 dark:border-gray-800 bg-gray-50/75 dark:bg-gray-800/50 text-[11px] font-bold text-gray-500 dark:text-gray-400">
                        <template x-for="m in actualMonths" :key="m"><th class="border-r border-gray-200/80 dark:border-gray-800 px-2 py-2 text-center uppercase" x-text="m"></th></template>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors">
                            <td class="border-r border-gray-100 dark:border-gray-800 px-4 py-2.5 text-center">
                                <a :href="detailUrl(row)" :title="row.status_entry === 'sudah' ? 'Update Entry' : 'Entry'"
                                    :class="row.status_entry === 'sudah' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-brand-500 hover:bg-brand-600'"
                            ...
                            <td class="border-r border-gray-100 dark:border-gray-800 px-4 py-2.5 font-medium text-gray-800 dark:text-gray-200">
                                <span x-text="row.cost_center_header"></span>
                                <span class="ml-2 rounded-md px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider" :class="row.status_entry === 'sudah' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'" x-text="row.status_entry"></span>
                            </td>
                            <template x-for="m in actualMonthKeys" :key="m"><td class="border-r border-gray-100 dark:border-gray-800 px-2 py-2 text-right font-mono" x-text="fmtActual(row.actual[m])"></td></template>
                            <td class="px-4 py-2.5 text-right font-bold text-gray-900 dark:text-white font-mono" x-text="fmtActual(row.total_actual)"></td>
                        </tr>
                    </template>
                </tbody>
                <tfoot>
                    <template x-if="entryAccounts.length > 0">
                        <tr class="bg-gray-50 dark:bg-gray-800/60 font-bold text-gray-900 dark:text-white border-t border-gray-200 dark:border-gray-700 text-xs">
                            ...
                            <template x-for="m in actualMonthKeys" :key="'ft_' + m"><td class="border-r border-gray-200 dark:border-gray-700 px-2 py-3 text-right font-mono" x-text="fmtActual(columnTotal(m))"></td></template>
                            <td class="px-4 py-3 text-right font-mono" x-text="fmtActual(grandTotal())"></td>
                        </tr>
                    </template>
                </tfoot>
            </table>
        ...
        <template x-if="entryAccounts.length > 0">
            <div class="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 dark:border-gray-800 p-3.5 sm:p-4 bg-gray-50/50 dark:bg-gray-800/30 text-xs text-gray-500 dark:text-gray-400">
                <span>Showing <span class="font-bold text-gray-800 dark:text-gray-200" x-text="((currentPage - 1) * perPage) + 1"></span> to <span class="font-bold text-gray-800 dark:text-gray-200" x-text="Math.min(currentPage * perPage, totalRows)"></span> of <span class="font-bold text-gray-800 dark:text-gray-200" x-text="totalRows"></span> entries</span>
                <div class="flex items-center gap-1" x-show="totalPages > 1">
                    ...
                    <template x-for="(p, pi) in pageNumbers()" :key="'pg_' + pi">
                        <button type="button" @click="goPage(p)" x-text="p" class="h-8 min-w-[32px] px-2 rounded-lg text-xs font-semibold transition-colors flex items-center justify-center" :class="currentPage === p ? 'bg-brand-500 text-white font-bold shadow-xs' : 'border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'"></button>
                    </template>
