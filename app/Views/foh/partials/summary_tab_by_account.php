<div class="space-y-6">
  ...
  <div class="flex items-center gap-3 bg-gray-50 p-4 rounded-xl border border-gray-200 dark:bg-gray-800 dark:border-gray-700 max-w-lg">
    ...
    <select x-model="selectedAccountViewMode" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs text-gray-700 shadow-sm focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white">
      <template x-for="mode in accountViewModes" :key="mode">
  ...
  <div class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
    <table class="w-full text-left text-xs text-gray-600 dark:text-gray-300">
      ...
      <tbody class="divide-y divide-gray-200 dark:divide-gray-800 font-mono text-[11px]">
        <template x-if="true">
          <tr>
            <td colspan="4" class="px-4 py-12 text-center text-gray-400 font-sans">
              Silakan tekan tombol cari untuk memuat data ringkasan per Account.
            </td>
          </tr>
        </template>
      </tbody>
