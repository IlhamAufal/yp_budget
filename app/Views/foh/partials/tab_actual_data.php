        ...
        <div class="relative" x-data="{ openCC: false, searchCC: '' }">
          ...
          <div x-show="openCC" @click.outside="openCC = false" x-transition class="absolute z-30 mt-1 w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg overflow-hidden">
            ...
            <ul class="max-h-60 overflow-auto py-1 text-xs text-gray-700 dark:text-gray-200 p-1">
              <li @click="selectedCostCenter = null; openCC = false; fetchActualData()" class="cursor-pointer px-3 py-2 rounded-lg hover:bg-brand-50 dark:hover:bg-gray-700 hover:text-brand-600 transition-colors">
                <span class="font-semibold">— Semua Cost Center —</span>
              </li>
              <template x-for="cc in filteredCostCenters(searchCC)" :key="cc.cost_center">
                <li @click="selectedCostCenter = cc; openCC = false; fetchActualData()" class="cursor-pointer px-3 py-2 rounded-lg hover:bg-brand-50 dark:hover:bg-gray-700 hover:text-brand-600 transition-colors">
      ...
      <table class="w-full text-left text-xs border-collapse min-w-[1000px] text-gray-600 dark:text-gray-300">
        <thead class="bg-brand-500 text-white text-xs font-semibold">
          <tr class="border-b border-brand-600 bg-brand-500 text-white font-semibold">
            <th class="border-r border-white/20 py-3 px-3 w-12 text-center text-white">No.</th>
            <th class="border-r border-white/20 py-3 px-4 min-w-[140px] text-white">Main Account</th>
            <th class="border-r border-white/20 py-3 px-4 min-w-[180px] text-white">Description</th>
            <template x-for="m in months" :key="m">
              <th class="border-r border-white/20 py-3 px-2 text-right text-white" x-text="m"></th>
            </template>
            <th class="py-3 px-4 text-right text-white bg-brand-600 font-semibold">Grand Total</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800 text-xs">
          <template x-if="actualTableData.length === 0 && !loading">
            <tr>
              <td :colspan="months.length + 4" class="py-12 px-4 text-center text-gray-400 dark:text-gray-500">
          ...
          <template x-for="(row, idx) in actualTableData" :key="idx">
            <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors">
              <td class="border-r border-gray-100 dark:border-gray-800 py-2.5 px-3 text-center text-gray-500" x-text="(currentPage - 1) * perPage + idx + 1"></td>
              <td class="border-r border-gray-100 dark:border-gray-800 py-2.5 px-4 font-mono font-bold text-gray-900 dark:text-white" x-text="row.main_account"></td>
              ...
              <template x-for="(m, mIdx) in months" :key="m">
                <td class="border-r border-gray-100 dark:border-gray-800 py-2.5 px-2 text-right font-mono" x-text="formatNumber(row[m.toLowerCase()] || row.monthly?.[mIdx + 1] || 0)"></td>
              </template>
